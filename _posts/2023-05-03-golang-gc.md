...
Preemption은 특정 실행의 제어권을 넘기는 것을 의미합니다.

### STW Implementation using Preemption

// STW 구현을 위해서 preemption이 필요하다는 내용

### Function Call Preemption

// Go 1.2에서 function call을 할 때 preemption check하는 기능이 추가되었다는 내용

// 이정도만 해도 쓸모있었지만, 몇몇 문제가 발생했고 특히 tight loop에서 function call이 없으면 loop가 끝날때까지 STW를 하지 못하여 latency가 올라가게 된다. 그리고 다른 고루틴들은 멈춰있기에 프로그램의 throuput이 즐어든다는 내용
